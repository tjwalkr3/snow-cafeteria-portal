...
public class StationSelectVM : IStationSelectVM
{
    ...
    public LocationDtoOld? SelectedLocation { get; private set; }
    public List<StationDtoOld>? Stations { get; private set; }

    public StationSelectVM(IApiMenuService menuService)
    {
        _menuService = menuService;
        Stations = new List<StationDtoOld>(); // Start with empty list
    }
    ...
    public async Task GetDataFromRouteParameters(string uri)
    {
        ...
        try
        {
            LocationDtoOld location = JsonSerializer.Deserialize<LocationDtoOld>(queryParams.Get("location") ?? string.Empty) ?? throw new ArgumentException("Failed to deserialize location from query parameter.");
